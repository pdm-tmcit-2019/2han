...
namespace WorewolfSharpGUI
{
    public class NextDeserializer
    {
        ...
        public void NextDay(string Day, string Phase)
        {
            ...
            {
                case "morning":
                    FilePath = Path.GetFullPath($"../../JsonFiles/DAY{Day}/server2client/morning.jsonld");
                    break;

                case "noon":
                    FilePath = Path.GetFullPath($"../../JsonFiles/DAY{Day}/server2client/noon.jsonld");
                    break;

                case "night":
                    FilePath = Path.GetFullPath($"../../JsonFiles/DAY{Day}/server2client/night.jsonld");
                    break;

                case "result":
                    FilePath = Path.GetFullPath($"../../JsonFiles/DAY{Day}/server2client/result.jsonld");
                    break;
            }
            ...
            for (int i = 0; i < jsonContract.Role.GetLength(0); i++)
            {
                switch (jsonContract.Role[i].Name.En)
                {
                    case "Villager":
                        ...
                        for (int j = 1; j < jsonContract.Role[i].Board.GetLength(0); j++) //日付にリンクして変える必要がある
                        {
                    ...
                    case "Seer":
                        ...
                        for (int j = 1; j < jsonContract.Role[i].Board.GetLength(0); j++)
                        {
                            if (jsonContract.Role[i].Board[j].polarity == "positive")
                            {
                                DatabaseBoard.Name = jsonContract.Role[i].Board[j].Character.Name.Ja;
